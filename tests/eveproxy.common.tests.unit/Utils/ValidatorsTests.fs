...
module ValidatorsTests =
    ...
    [<Property(Arbitrary = [| typeof<AlphaNumericString> |], Verbose = true)>]
    let ``isUrl - invalid returns false`` (value: string) = value |> Validators.isUrl |> not

    [<Property(Arbitrary = [| typeof<UrlString> |], Verbose = true)>]
    let ``isUrl - valid returns true`` (value: string) = value |> Validators.isUrl
    ...
    [<Property(Arbitrary = [| typeof<AlphaNumericString> |], Verbose = true)>]
    let ``isNonEmptyString - valid returns true`` (value: string) = value |> Validators.isNonEmptyString
